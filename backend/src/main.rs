...
async fn main() -> std::io::Result<()> {
    ...
    let server = HttpServer::new(move || {
        ...
        let cors = Cors::default()
            .allow_any_method()
            .allowed_origin_fn(|header, _| header.to_str().unwrap().starts_with("http://localhost"))
    ...
    let cors = Cors::default()
        .allowed_origin_fn(|origin, _req| origin.to_str().unwrap().starts_with("http://localhost"))
        .allow_any_origin()
        .allow_any_header()
        .expose_any_header()
        .allow_any_method()
